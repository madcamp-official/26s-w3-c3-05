...
# --- max_active_frames 캡: 과도하게 긴 ACTIVE를 끊는다 (2026-07-21) ---


def test_active_capped_after_max_frames() -> None:
    """활성이 계속 이어져도 max_active_frames에 도달하면 강제로 ENDING한다."""
    spotter = GestureSpotter(_config(min_consecutive_frames=1, max_active_frames=3))
    seen = []
    for i in range(4):
        est = spotter.push(_prediction(), timestamp_ms=i * 10, frame_id=i)
        seen.append(est.phase)
    # ONSET(1) → ACTIVE(2) → 3번째 활성 프레임에서 캡 도달 → ENDING → 리셋(IDLE)
    assert seen == [
        GesturePhase.ONSET,
        GesturePhase.ACTIVE,
        GesturePhase.ENDING,
...
def test_cap_ending_blocks_refire_until_background_seen() -> None:
    """캡으로 끊긴 직후, 활성이 계속돼도(같은 제스처가 window에 잔류) 배경을 한 번
    볼 때까지 새 이벤트를 시작하지 않는다 — machine-gun 재발화 방지."""
    spotter = GestureSpotter(_config(min_consecutive_frames=1, max_active_frames=3))
    # 3프레임째에 캡 ENDING
    for i in range(3):
        spotter.push(_prediction(), timestamp_ms=i * 10, frame_id=i)
    assert spotter.state == GesturePhase.IDLE

    # 활성이 계속돼도 재무장 보류 상태 → 새 ONSET/ENDING 없음
    refire_phases = []
    for i in range(3, 8):
        est = spotter.push(_prediction(), timestamp_ms=i * 10, frame_id=i)
        refire_phases.append(est.phase)
    assert all(p == GesturePhase.IDLE for p in refire_phases)

    # 배경(비활성)을 한 번 확정하면 재무장 → 다음 활성부터 정상 ONSET
    spotter.push(_inactive(), timestamp_ms=80, frame_id=8)
    est = spotter.push(_prediction(), timestamp_ms=90, frame_id=9)
    assert est.phase == GesturePhase.ONSET


def test_natural_ending_does_not_block_refire() -> None:
    """자연 종료(확신 하락·배경 복귀)는 재무장 보류를 걸지 않는다 — 캡 종료만 보류."""
    spotter = GestureSpotter(_config(min_consecutive_frames=1, max_active_frames=18))
    spotter.push(_prediction(), timestamp_ms=0, frame_id=0)  # ONSET
    spotter.push(_prediction(), timestamp_ms=10, frame_id=1)  # ACTIVE
    end = spotter.push(_inactive(), timestamp_ms=20, frame_id=2)  # 자연 ENDING
    assert end.phase == GesturePhase.ENDING
    # 곧바로 활성이 오면 바로 다음 이벤트를 시작할 수 있어야 한다(보류 없음).
    onset = spotter.push(_prediction(), timestamp_ms=30, frame_id=3)
    assert onset.phase == GesturePhase.ONSET


def test_cap_zero_disables_limit() -> None:
    """max_active_frames=0이면 캡이 없어 활성이 이어지는 한 ACTIVE를 유지한다."""
    spotter = GestureSpotter(_config(min_consecutive_frames=1, max_active_frames=0))
    for i in range(40):
        est = spotter.push(_prediction(), timestamp_ms=i * 10, frame_id=i)
    assert est.phase == GesturePhase.ACTIVE
    assert spotter.state == GesturePhase.ACTIVE


def test_max_active_frames_validation() -> None:
    import pytest

    with pytest.raises(ValueError):
        SpotterConfig(max_active_frames=-1)
    with pytest.raises(ValueError):
        SpotterConfig(max_active_frames=1)  # 0 또는 2 이상만 허용
