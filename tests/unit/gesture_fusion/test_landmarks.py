...
def _points_with_scale(scale: float, offset: tuple[float, float, float] = (0.0, 0.0, 0.0)) -> np.ndarray:
    ...
    """
    points = np.zeros((HAND_LANDMARK_COUNT, 3), dtype=np.float64)
    for i in range(HAND_LANDMARK_COUNT):
        points[i] = [offset[0] + i * 0.05 * scale, offset[1], offset[2]]
    points[WRIST] = offset
    points[MIDDLE_FINGER_MCP] = [offset[0] + scale, offset[1], offset[2]]
    return points
...
def test_wrist_becomes_origin() -> None:
    raw = _raw(_points_with_scale(0.2, offset=(0.5, 0.3, 0.1)))
    obs = normalize_hand(raw)
    assert obs.hand_detected
    np.testing.assert_allclose(obs.landmarks[WRIST], [0.0, 0.0, 0.0], atol=1e-9)


def test_scale_invariance_same_shape_different_distance() -> None:
    """카메라 거리(스케일)만 다르고 손 모양이 같으면 정규화 결과가 같아야 한다."""
    near = normalize_hand(_raw(_points_with_scale(0.4, offset=(0.1, 0.1, 0.0))))
    far = normalize_hand(_raw(_points_with_scale(0.1, offset=(0.6, 0.2, 0.0))))
...
def test_palm_scale_is_root_to_tip_distance() -> None:
    ...
    np.testing.assert_allclose(obs.landmarks[MIDDLE_FINGER_MCP], [1.0, 0.0, 0.0], atol=1e-9)
...
def test_rotation_is_preserved_not_normalized() -> None:
    ...
    rotated[MIDDLE_FINGER_MCP] = [rotated[WRIST][0], rotated[WRIST][1] + 0.2, rotated[WRIST][2]]
...
def test_configurable_palm_reference_indices() -> None:
    ...
    points = _points_with_scale(0.2)
    points[1] = [points[WRIST][0] + 0.5, points[WRIST][1], points[WRIST][2]]
...
def test_observation_rejects_non_finite_landmarks() -> None:
    bad = np.zeros((HAND_LANDMARK_COUNT, 3), dtype=np.float64)
    bad[0, 0] = np.nan
