...
Commit 조건 7(동일 이벤트 재실행 방지)과 `intent_id` 생성은 task 7, 실제
`jarvis.contracts.Intent` 조립(capability/operation/value 매핑)은 task 8이
담당한다 — 이 모듈은 "커밋해도 되는가"만 결정하고 무엇을 커밋할지는 만들지 않는다.
"""
...
class FusionConfig:
    ...
    Task 7의 intent 단위 중복 방지(동일 이벤트 재실행 금지)와는 다른 관심사다 —
    여기서는 "너무 가까운 시간 안에 또 커밋하지 말라"는 시간 기반 최소 간격만
    보장한다.
    """
