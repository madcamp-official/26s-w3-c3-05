...
def _print_report(report: ClassificationReport) -> None:
    ...
    for cls, f1 in sorted(report.per_class_f1.items()):
        print(f"  {DEFAULT_GESTURE_LABELS[cls]}: {f1:.4f}")
    print("혼동행렬 (행=정답, 열=예측):")
    header = "".join(f"{label[:8]:>9}" for label in DEFAULT_GESTURE_LABELS)
    print(" " * 9 + header)
    for i, row in enumerate(report.confusion):
        row_str = "".join(f"{int(v):>9d}" for v in row)
        print(f"{DEFAULT_GESTURE_LABELS[i][:8]:>9}{row_str}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = evaluate(args.checkpoint, args.data_root)
