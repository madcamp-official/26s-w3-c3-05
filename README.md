...
모든 프로브는 **가짜 결과를 지어내지 않는다**. mediapipe·torch·모델 파일이 없거나 체크포인트가 미학습(`trained=false`)이면 인식을 켜지 않고 그 사유를 화면에 남긴다. 시연 탭도 판정과 실행을 분리해, 실행을 끈 채로 판정만 관찰할 수 있다. 전구 그림은 `BulbPoller`가 2초마다 `getPilot`으로 읽어온 **실물 색**이며, 한 번도 조회에 성공하지 못했으면 마지막으로 보낸 명령을 지어내지 않고 꺼짐과 같은 회색으로 둔다. 옆의 수치는 실물에서 읽었는지 보낸 명령 기준인지를 문구로 구분해 표시하고, 실물 성공 여부는 `ExecutionOutcome` 배지로 따로 보여준다.
...
```bash
pip install -e ".[vision,ml,ui]"
python -m jarvis.monitoring.cli --demo   # 저장소 루트에서 실행 (--demo: '시연' 탭으로 시작)
```

저장소 루트에서 `python -m`으로 실행한다(이유는 12장). 주요 옵션: `--camera N`(카메라 인덱스), `--no-gaze`(gaze 추론을 끄고 손만 — 지연 진단용).

## 전구 연결 (선택)

`.env.example`을 `.env`로 복사하고 전구 주소만 채운다. 계정·토큰은 필요 없다 — 같은 LAN의 UDP로 직접 제어한다.

```bash
WIZ_DEVICE_TARGETS={"room.bulb":"9877d5cffaf8@10.26.34.165"}
```

`MAC@IP` 형식을 권장한다. 평소엔 IP로 곧장 쏘고, DHCP로 주소가 바뀌었을 때만 MAC으로 재탐색한다. MAC만 적으면 브로드캐스트 탐색에 의존하는데, 이를 차단하는 네트워크가 있다. 앱은 시작하자마자 전구에 한 번 닿아 보고 그 결과를 시연 탭 로그에 남긴다.

## 처음 한 번 — 기기 등록

1. **시연** 탭의 `물체 등록`으로 노트북과 전구를 각각 등록한다. 이름을 넣고 **기종**을 `computer` / `electric bulb`로 고르면 되며, 이 기종이 곧 런타임 기기(`laptop`·`room.bulb`)로 자동 연결된다. 등록된 물체는 같은 화면의 목록에서 `재등록`·`이름 변경`·`삭제`로 관리한다.
2. `타깃 고정` 체크를 **해제**한다. 이 옵션은 시선을 무시하고 고른 기기에만 명령을 보내는 안전 폴백이며, **기본값이 켜짐 + `laptop`이다** — 켜 둔 채로는 전구를 아무리 바라봐도 명령이 노트북으로 간다.

## 조작

기기를 **바라보면** 선택되고(dwell 후 lock), 그 상태에서 **제스처**를 하면 실행된다. 같은 제스처도 바라보는 기기에 따라 다른 동작이다.

| 제스처 | 노트북 | 전구 |
| --- | --- | --- |
| 두 손가락 슬라이드 ↑↓ | 스크롤 | 밝기 ±30 |
| 두 손가락 슬라이드 ←→ | 가상 데스크톱 전환 | 밝기 ±30 |
| 손목 회전 ↻↺ | 볼륨 | 색상 ±60° |
| 활짝 편 손바닥 | — | 전원 on/off |

노트북 대상 명령은 `손동작으로 컴퓨터 제어` 토글도 함께 지켜야 실행된다. 전구를 바라보는 동안에는 커서·클릭 같은 정적 pose 제어가 자동으로 멈춘다(모드 전환 조작 없이 시선만으로 갈린다).

## 잘 안 될 때

시연 탭 웹캠 아래의 **로그가 차단 사유를 한국어로 그대로 찍는다.** 추측하기 전에 그 줄을 먼저 본다.

| 로그 | 원인 |
| --- | --- |
| `커밋 … → laptop`만 반복 | `타깃 고정`이 켜져 있다(위 2번) |
| `바라보는 기기 없음` | lock 미형성 — 등록 시 기종을 골랐는지 확인하거나 임계값을 `느슨`으로 |
| `제스처가 기기 선택보다 먼저 시작` | 먼저 응시해 lock을 잡은 **뒤에** 제스처를 시작한다 |
| `전구 미설정` / `전구 연결 실패` | `.env`를 못 읽거나(루트에서 실행했는지) 전구가 다른 네트워크에 있다 |

임계값이 빡빡해 잘 안 잡히면 `임계값` 콤보를 `느슨`으로 바꾼다(프리셋을 바꾸면 현재 lock은 초기화된다).
