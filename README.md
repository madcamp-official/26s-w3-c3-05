...
지원 동작: 커서 이동(`index_point`), 클릭·드래그·더블클릭(`pinch_index`, 마우스 down/up 모델), 우클릭(`pinch_middle`), 스크롤(`two_fingers`가 가리키는 방향), 탭 닫기(`middle_point`, dwell 500ms), 재생/일시정지(`ok`, 엄지-검지 원), 주먹→보 전이로 Task View(Windows)·Mission Control(macOS), 검지 회전으로 볼륨 조절. 상세는 [gesture-fusion.md](documents/gesture-fusion.md) "정적 손 자세 파이프라인" 참조.
