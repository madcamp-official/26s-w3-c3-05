...
| 2026-07-19 | (미해결·논의필요) `normalize_hand`의 손목 원점화가 매 프레임 손목을 `(0,0,0)`으로 만들어, 손 모양·회전이 그대로인 순수 평행이동(swipe)에서는 position·velocity·acceleration이 21개 랜드마크 전부 이론상 0이 됨 — swipe_up/down/left/right를 구분할 유일한 신호(손 전체 이동 방향)가 현재 feature 벡터에 없음. 수정 제안: `HandObservation`에 `wrist_position`(= `raw.points[origin_index] / palm_scale`, 원점화 없이 palm_scale로만 정규화) 필드를 추가하고, `HandFeatureExtractor`가 기존 `landmarks`와 별도로 이를 미분해 `wrist_velocity`/`wrist_acceleration`을 새 `GestureConfig.include_wrist_translation` 그룹으로 추가(기존 4개 그룹 뒤에 이어붙여 순수 추가, 기존 테스트 영향 없음) | 학습 파이프라인 설계 중 발견: 데이터·학습으로 고칠 수 없는 구조적 신호 손실이라 학습 시작 전 해결 필요. rotate_clockwise/counter_clockwise는 회전이 보존돼 영향 없음 | 3인 제기, 2인(features.py·landmarks.py 소유자, 현재 랜드마크 추출 재작성 중)과 협의 대상 |
