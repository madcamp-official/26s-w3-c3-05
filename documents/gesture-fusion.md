...
- **9-class**(2026-07-22): `index_point`·`middle_point`·`pinch_index`·`pinch_middle`·`two_fingers`·`open_palm`·`fist`·`ok` + `none`. 참고 레포와 같은 규모의 MLP(입력→20→10→클래스), 노트북 CPU로 수 초 학습. `ok`(엄지-검지 원 → 재생/일시정지)를 새로 추가하며 데이터를 누적 수집했다(실험 중 `fist`를 잠시 none으로 흡수해 8-class로 갔다가, fist·ok를 **함께** 두는 9-class로 데이터를 재구성). 활성 모델 `models/hand_pose_classifier.pt` = **9-class, 전체 94.6%**(index_point 94.9 / middle_point 88.9 / pinch_index 98.2 / pinch_middle 96.2 / two_fingers 100 / open_palm 93.1 / fist 98.8 / ok 100 / none 91.2 — 에피소드 홀드아웃). 이력: 7-class(87.1%, `pinch_middle` 68% 정체) → 8-class v2(94.4%, `middle_point` 추가·재수집으로 pinch_middle 68→99.4% 해소) → 9-class(ok 추가). 이전 가중치는 저장소에 보존하지 않는다(수집 데이터·백업은 `.claude/tmp`에 로컬 보관). (아래 82.6%→92.3% 등은 7-class 초기 실측 이력.)
...
동작: 커서 이동(index_point·pinch_index, 마우스식 상대 이동+포인터 가속+soft deadzone+palm_scale 정규화, 참조점은 손목의 **평활된 이미지 좌표**; `index_point`는 **검지 직진도 게이트**(`INDEX_STRAIGHTNESS_MIN`=0.97) 미만이면 애매하게 굽힌 손이 index_point로 오분류돼도 커서 이동에 진입하지 않음), **클릭/드래그·더블클릭은 마우스 down/up 모델**(핀치 진입=버튼 down, 릴리즈=up — 짧게 쥐면 클릭·길게 쥐고 이동하면 드래그가 OS 판정으로 자연히 갈려 유지시간 분기가 불필요; 빠른 두 번 핀치는 두 **진입** 간격(`DOUBLE_CLICK_MS`=800ms) 이내면 두 번째 down/up에 `clickState`=2를 실어 macOS가 더블클릭으로 합침 — 단일 클릭 두 번만으론 macOS가 더블클릭을 인식하지 않아서), 우클릭(pinch_middle), 스크롤(two_fingers가 **가리키는 방향** — 손 이동이 아니라 MCP→끝 벡터라 손을 멈춰도 유지, 수직성 `MIN_VERTICALITY`=cos(30°)(≈0.866) 미만(=수직에서 30° 넘게 기울면) 방향 안 지어냄; 손가락이 접히면 **직진도(straightness) 게이트**(`TWO_FINGER_STRAIGHTNESS_MIN`=0.85, 두 손가락 중 덜 편 쪽)로 스크롤을 끊어, 멈추려 주먹 쥘 때 손끝이 아래로 스윙해 역방향 스크롤이 튀는 것을 막음 — 직진도 = MCP→끝 직선거리 / 관절 세그먼트 합(1.0=곧음)이라 손 크기·거리·기울기에 불변, 구 지표(MCP→끝 거리 `MIN_FINGER_EXTENSION`)보다 강건), 손 펴기(fist→open_palm 전이)로 **Mission Control**(macOS, `open -a`)·**Task View**(Windows, Win+Tab) 열기, 중지만 폄(`middle_point`)으로 **탭 닫기**(macOS Cmd+W·Windows Ctrl+W, 진입 시 1회 발화, dwell 500ms — 파괴적이라 기본보다 길게; **디버깅 툴 창이 활성이면 툴 자체를 닫는다**), **OK 사인**(`ok`, 엄지-검지 원)으로 **재생/일시정지**(진입 시 1회, macOS는 NX_KEYTYPE_PLAY 미디어 키), 검지를 편 채 **회전**(MCP→TIP 방향의 누적 각도)으로 **볼륨**(방향↔증감 부호는 `ROT_SIGN`, `ROT_STEP_DEG`=60°당 1스텝이라 빠를수록 초당 스텝↑ = 비례 제어; `ROT_MIN_SPEED`=60°/s 미만은 무시해 포인팅 지터·손 평행이동과 분리; **워밍업 게이트** `ROT_ACTIVATION_DEG`=270° — 한 회전 세션에서 순(net) 회전이 약 3/4바퀴를 넘어야 볼륨이 시작돼, 일상 동작의 작은 회전이 갑자기 볼륨을 바꾸는 것을 막음). 게이트는 확정 상태가 아니라 **분류기 라벨(index_point)**이라 **낮은 tilt로 먼저 진입할 필요 없이 손을 눕힌 채 바로 회전을 시작**할 수 있다(고tilt에선 신뢰 게이트에 걸려 상태 진입은 못 하지만 라벨은 유지 — 실측 88%). 회전 추적은 trusted 게이트 **앞**에서 돈다. 회전을 감지하면(워밍업 중 포함) `ROT_HOLD_MS`(400ms) 동안 **볼륨 노브 모드**를 유지해 다른 모든 동작(클릭·드래그·커서·스크롤)을 막는다 — 게이지를 채우는 동안에도 회전 중 순간 오인식(pinch_index·fist)이 클릭/드래그로 새던 문제를 없앤다. 임계는 대부분 **시간**이라 투영에 흔들리지 않는다(기하학적 임계는 손 각도에 8.85배까지 흔들렸다) — 직진도 게이트만은 palm_scale 정규화 좌표라 손 거리·각도에 강건하다.
...
- **모든 동작이 Windows·macOS 양쪽에서 대응**된다(클릭·우클릭·드래그·스크롤·커서, 탭 닫기(Cmd/Ctrl+W)·볼륨(VOLUME_UP/DOWN)·재생/일시정지(PLAY_PAUSE, OK 사인)·주먹→보 전이(macOS=Mission Control, Windows=Task View)). 클릭은 버튼 down/up을 분리해 보내고(핀치 진입/릴리즈에 대응), 더블클릭은 두 번째 down/up에 `clickState`=2를 실어 macOS가 합치게 한다(Windows는 두 클릭의 간격으로 OS가 판정 — clickState 무시). `Win32InputSink`·`MacOSInputSink`가 같은 `InputSink` Protocol을 구현하고 `default_input_sink()`가 플랫폼으로 고른다.
...
- **주먹→보 전이 키는 플랫폼별**: macOS=Mission Control(`open -a`), Windows=Task View(Win+Tab)(`_transition_key()`가 `sys.platform`으로 선택, 2026-07-21에 F11/재생·일시정지에서 교체). 스크롤은 30fps 그대로면 감당이 안 돼 `SCROLL_INTERVAL_MS`(60ms)로 솎아내며, 이 스로틀은 실행·UI 기록 양쪽에 적용한다.
...
- [x] **Task 3 — Causal TCN/GRU** (`model_protocol.py`, `model.py`): dilated causal 1D conv(TCN), `GestureModel` Protocol(torch 무의존, `mediapipe_hands.py`와 같은 격리 원칙)로 아키텍처 교체 가능. `phase`는 `jarvis.contracts.GesturePhase`를 그대로 재사용(자체 enum 재정의 없음). gesture head(10-class: none + rotate 2종 + slide_two_fingers 4종 + drumming_fingers + doing_other_things + stop_sign) + phase head(4-class), confidence=softmax max, uncertainty=정규화 엔트로피. 진짜 인과성(미래 프레임 미사용)을 `test_output_is_truly_causal`로 회귀 검증. **학습 완료**(2026-07-22, `models/gesture_tcn_finetuned.pt`, `ModelMetadata.trained=True`, finetune-epoch82) — 실시간 탭에 인식 스트림으로 연결됨. 검증 후 수정 반영: torch(`ml` extra) 미설치 환경에서도 테스트 스위트가 수집되도록 `test_model*`에 `importorskip` 가드 추가, 표준 `.[dev]`(torch 없음) 타입체크 통과를 위해 `model` 모듈 mypy `disallow_subclassing_any` 예외, `load_weights`의 `torch.load`에 `weights_only=True`(pickle 코드 실행 차단).
...
- [x] **Task 8 — Intent 조립·출력** (`intent.py`, `configs/gesture_capability_map.json`): `GestureCapabilityMap`이 `(target device_id, gesture)` → capability/operation/value 매핑을 JSON에서 로드(코드가 아니라 config로 관리, 새 제스처·기기는 파일만 수정). 같은 제스처도 기기별로 다르게 매핑됨(노트북 slide_two_fingers_down=scroll, 전구 slide_two_fingers_down=brightness — README 9·15장). `assemble_intent`가 task 6·7의 `CommitDecision`을 받아 `jarvis.contracts.Intent`(계약 §3)를 최종 조립, 매핑 없는 조합은 `None`(거부). **통합 갭 해소(dev-3 구현)**: README 15장 필수 시나리오 "노트북 Swipe Left"용 `desktop_switch` capability를 3인의 `WindowsAdapter`(`_execute`)가 처리한다(`_desktop_switch` → Windows Ctrl+Win+화살표 hold, macOS는 osascript로 System Events에 Ctrl+화살표 키 코드(합성 CGEvent·SkyLight는 실측상 무시·아티팩트라 폐기, documents/runtime-protocol.md·decisions.md 기록), 방향은 제스처와 반대(오른쪽=왼쪽 데스크톱), `test_desktop_switch_*` 회귀 포함). 2026-07-22에 창 전환(Alt+Tab/Cmd+Tab, `window_switch`)에서 가상 데스크톱 전환으로 바꾸고 capability 이름도 개명했다 — 좌우 슬라이드의 공간적 은유에 맞춤(documents/decisions.md 기록).
...
Task 1~9(hand landmark → hard-negative mining) 전부 구현·테스트·문서화 완료. gesture_fusion 단위 테스트 251개 통과(저장소 전체는 그 이상), ruff clean. 실제 학습 데이터·모델 가중치(`ModelMetadata.trained=True`)는 별도 데이터 확보 후 진행 필요.
