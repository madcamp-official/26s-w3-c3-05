...
- **모든 동작이 Windows·macOS 양쪽에서 대응**된다(클릭·우클릭·드래그·스크롤·커서, 탭 닫기(Cmd/Ctrl+W)·볼륨(VOLUME_UP/DOWN)·주먹→보 전이(macOS=Mission Control, Windows=Task View)). `Win32InputSink`·`MacOSInputSink`가 같은 `InputSink` Protocol을 구현하고 `default_input_sink()`가 플랫폼으로 고른다.
