...
1. **진입은 느리게(dwell: 핀치 클릭류 80ms, 그 외 자세 기본 120ms), 이탈은 빠르게 하되 관대하게**: 자세가 유지돼야 상태 진입(전이 프레임은 짧아 걸러짐). `none`이 몇 프레임 들어와도 3프레임까지는 안 끊는다(놓침 15.4%가 조작 중단으로 이어지면 안 됨). 30fps 프레임 양자화로 실질 유지 시간은 명목값 + 최대 33ms.
