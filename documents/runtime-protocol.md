...
- `windows.py`: `WindowsAdapter`가 discrete command(scroll/volume/media/**desktop_switch**)를 `InputSink`로 매핑. 로컬 합성 입력은 OS가 받아들이지만 효과를 되읽지 않으므로 성공은 `ACKNOWLEDGED`가 정직한 상한(VERIFIED 위조 안 함). 처리 못 하는 capability/operation은 추측 없이 `FAILED`. `Win32InputSink`는 user32(keybd_event/mouse_event) 하드웨어 경계로 `ctypes` lazy import — 실물 검증 필요(자동 테스트는 fake sink 사용).
  - **desktop_switch(2026-07-22)**: `_desktop_switch`가 `switch_desktop(forward, count)`로 가상 데스크톱을 전환. Windows는 **Ctrl+Win+화살표**(hold), macOS `MacOSInputSink`는 **osascript로 System Events에 Ctrl+화살표 키 코드**를 보낸다(Space 이동). **트리거 변경(2026-07-22)**: 이제 노트북 데스크톱 전환은 동적 slide 제스처(capability map)가 아니라 **정적 two_fingers 좌우 스와이프**가 `pose_control`을 통해 같은 `sink.switch_desktop`을 직접 호출한다 — `desktop_switch` capability와 어댑터의 `_desktop_switch`는 그대로 두되 현재 어떤 제스처에도 매핑되지 않는다(laptop `{}`). OS 합성 방식은 정적·동적 경로가 동일. 처음엔 창 전환(`window_switch`, Alt+Tab/Cmd+Tab)으로 구현했으나 좌우 슬라이드의 공간적 은유("옆으로 넘긴다")에 맞춰 데스크톱 전환으로 바꾸고 capability 이름도 개명했다(사용자 지시).
    - **macOS 구현이 System Events인 이유(실측)**: 프로세스가 직접 만든 CGEvent 합성 키는 커서(합성 마우스)·물리 키는 되는데도 Space 전환만 WindowServer가 조용히 무시했다(HID·session·annotated tap, flag-only, 명시적 이벤트 소스 전부 실패). SkyLight 직접 전환(`CGSManagedDisplaySetCurrentSpace`)은 되지만 창↔Space 소속이 꼬여 미션 컨트롤에 창이 바탕으로 남는 아티팩트가 생긴다. **Apple 자체 프로세스인 System Events가 보낸 키만 진짜 단축키로 인정**돼 네이티브 그대로(애니메이션·창 관리 정상) 전환됐다. "Space 이동" 단축키(기본 Ctrl+←/→)와 **System Events 자동화 권한**에 의존(첫 호출 시 프롬프트). 끝 데스크톱에서는 새로 만들지 않고 아무 일도 안 한다(native Ctrl+화살표와 동일).
    - **방향(사용자 지시)**: 제스처와 **반대**로 매핑한다 — 오른쪽 슬라이드(forward)=왼쪽 데스크톱, 왼쪽 슬라이드=오른쪽 데스크톱. macOS·Windows 어댑터 양쪽 동일.
